...
def read_raw_physio_file(
    ...
    """
    Read a Bonn raw physiology ``.acq`` file.

    This is the reference implementation for physiology reading. A single
    ``.acq`` file is read using ``biopac.read_acq_file`` and converted into
    a tabular representation together with sampling rate and selected
    channel metadata.
...
def find_physio_acq_file(
    ...
    """
    Locate the Bonn raw physiology ``.acq`` file for one participant.

    This reference implementation expects a fixed file naming convention of
    the form ``"CALINET_Template{subject_id}.acq"`` located directly inside
    ...
    -------
    physio_path : str
        Full path to the expected Bonn ``.acq`` file.
    ...
    FileNotFoundError
        Raised if the expected ``.acq`` file is not found.

    Notes
    -----
    This function does not read or write files.

    File discovery is strict and assumes the Bonn template naming
    convention.
    """
    ...
    subject_id = subject_name.split("-")[1]
    expected_fname = f"CALINET_Template{subject_id}.acq"

    file_name = os.path.join(subject_raw_data_path, expected_fname)
